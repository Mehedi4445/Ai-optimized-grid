...
def dark_chart_layout(fig, height=300, **kw):
    """Apply dark theme layout. Merges xaxis/yaxis/legend dicts safely."""
    base_xaxis  = dict(gridcolor="#D0D7E2")
    base_yaxis  = dict(gridcolor="#D0D7E2")
    base_legend = dict()
    base_margin = dict(t=20, b=40, l=40, r=10)
    # Merge caller overrides into base dicts (no duplicate-kwarg error)
    if "xaxis"  in kw: base_xaxis.update(kw.pop("xaxis"))
    if "yaxis"  in kw: base_yaxis.update(kw.pop("yaxis"))
    if "legend" in kw: base_legend = kw.pop("legend")
    ...
    fig.update_layout(
        ...
        font=dict(family="Arial"), margin=base_margin,
        **({} if not base_legend else {"legend": base_legend}),
        **kw)
    return fig
...
def page_dashboard(user, api_key, city):
    ...
    with ca:
        if stats:
            ...
            fig.add_trace(go.Scatter(x=df["timestamp"],y=df["solar_generation"],
                name="Solar Gen (kW)",line=dict(color="#D4870A",width=2.5),
                fill="tozeroy",fillcolor="rgba(212,135,10,0.12)"))
            fig.add_trace(go.Scatter(x=df["timestamp"],y=df["total_load"],
                name="Total Load (kW)",line=dict(color="#0066CC",width=2,dash="dot")))
    ...
    with cb:
        fig_g=go.Figure(go.Indicator(mode="gauge+number+delta",value=snap["battery_level"],
            title={"text":"Battery %","font":{"color":"#5A6A7E","family":"Arial","size":12}},
            delta={"reference":50,"increasing":{"color":"#1A8C4E"},"decreasing":{"color":"#CC1A33"}},
            gauge={"axis":{"range":[0,100]},"bar":{"color":bc},"bgcolor":"#F5F7FA","borderwidth":0,
                   "steps":[{"range":[0,20],"color":"rgba(204,26,51,0.12)"},
                             {"range":[20,50],"color":"rgba(212,135,10,0.15)"},
                             {"range":[50,100],"color":"rgba(26,140,78,0.10)"}],
                   "threshold":{"line":{"color":"#0066CC","width":3},"value":80}},
            number={"suffix":"%","font":{"color":bc,"family":"Arial"}}))
        fig_g.update_layout(height=280,paper_bgcolor="#FFFFFF",font_color="#1A1A2E",
    ...
    with cc:
        fig_p=go.Figure(go.Pie(
            ...
            hole=0.55,marker=dict(colors=["#CC1A33","#D4870A","#0066CC","#1A8C4E"],
                                  line=dict(color="#FFFFFF",width=2)),
            textfont=dict(family="Arial",size=13)))
        fig_p.add_annotation(text=f"{snap['total_load']:.1f} kW",showarrow=False,
            font=dict(size=15,color="#D4870A",family="Arial"),y=0.5)
        fig_p.update_layout(title=dict(text="Load Distribution",font=dict(color="#5A6A7E",family="Arial",size=11)),
    ...
    with cd:
        if stats and len(stats)>2:
            ...
            fig_b=go.Figure(go.Scatter(x=df2["timestamp"],y=df2["battery_level"],
                ...
                fill="tozeroy",fillcolor="rgba(26,140,78,0.08)"))
            dark_chart_layout(fig_b,300,yaxis=dict(gridcolor="#D0D7E2",range=[0,100]))
            fig_b.update_layout(title=dict(text="Battery History",font=dict(color="#5A6A7E",family="Arial",size=11)))
...
def page_user(user):
    ...
    fig=go.Figure(go.Bar(x=df["day"],y=df["energy_consumed"],
        marker_color="#D4870A",marker_line_color="#CC5500",marker_line_width=1,
        text=df["energy_consumed"].round(1),textposition="outside",
        textfont=dict(color="#1A1A2E",size=10)))
    dark_chart_layout(fig,280,yaxis=dict(gridcolor="#D0D7E2",title="kWh"))
    st.plotly_chart(fig,use_container_width=True)
    ...
    with b2:
        fig2=go.Figure(go.Pie(
            ...
            hole=0.5,marker=dict(colors=["#CC5500","#0066CC","#1A8C4E"],
                                  line=dict(color="#FFFFFF",width=2)),
            textfont=dict(family="Arial")))
        fig2.update_layout(height=260,paper_bgcolor="#FFFFFF",font_color="#1A1A2E",
...
def page_admin(user):
    ...
    if usage:
        ...
        with cc2:
            fig=go.Figure(go.Bar(x=df_use["username"],y=df_use["total"],
                marker=dict(color=df_use["total"],colorscale=[[0,"#E8EDF5"],[0.5,"#D4870A"],[1,"#CC5500"]]),
                text=df_use["total"].round(1),textposition="outside"))
            dark_chart_layout(fig,300,yaxis=dict(gridcolor="#D0D7E2",title="kWh"))
            st.plotly_chart(fig,use_container_width=True)
        ...
        if not df_owed.empty:
            ...
            fig_bill = go.Figure(go.Bar(
                ...
                marker=dict(color=df_owed["outstanding"],
                            colorscale=[[0,"#D4870A"],[1,"#CC1A33"]],
                            line=dict(color="#FFFFFF",width=1)),
                text=[f"${v:.2f}" for v in df_owed["outstanding"]],
                textposition="outside",
                textfont=dict(color="#1A1A2E",family="Arial")
            ))
            dark_chart_layout(fig_bill, 280,
                yaxis=dict(gridcolor="#D0D7E2", title="Outstanding ($)"))
            st.plotly_chart(fig_bill, use_container_width=True)

            # Paid vs outstanding stacked bar
            fig_stack = go.Figure()
            fig_stack.add_trace(go.Bar(
                name="Paid", x=df_bill["username"], y=df_bill["paid"],
                marker_color="#1A8C4E", opacity=0.85
            ))
            fig_stack.add_trace(go.Bar(
                name="Outstanding", x=df_bill["username"], y=df_bill["outstanding"],
                marker_color="#CC1A33", opacity=0.85
            ))
            fig_stack.update_layout(barmode="stack")
            dark_chart_layout(fig_stack, 280,
                yaxis=dict(gridcolor="#D0D7E2", title="Amount ($)"),
    ...
    if stats:
        ...
        fig2.add_trace(go.Scatter(x=df_s["timestamp"],y=df_s["solar_generation"],
            name="Solar (kW)",line=dict(color="#D4870A",width=2)))
        fig2.add_trace(go.Scatter(x=df_s["timestamp"],y=df_s["total_load"],
            name="Load (kW)",line=dict(color="#0066CC",width=2,dash="dot")))
        fig2.add_trace(go.Scatter(x=df_s["timestamp"],y=df_s["battery_level"],
            name="Battery %",line=dict(color="#1A8C4E",width=2),yaxis="y2"))
        ...
        fig3=go.Figure(go.Pie(labels=ac_cnt["action"],values=ac_cnt["count"],hole=0.5,
            marker=dict(colors=["#CC1A33","#D4870A","#0066CC","#1A8C4E"],
                        line=dict(color="#FFFFFF",width=2)),
            textfont=dict(family="Arial")))
        fig3.update_layout(
            title=dict(text="Distribution Action Frequency",font=dict(color="#5A6A7E",family="Arial",size=11)),
            height=300,paper_bgcolor="#FFFFFF",font_color="#1A1A2E",margin=dict(t=40,b=10,l=10,r=10))
...
def page_distribution(user, api_key, city):
    ...
    with cp:
        ...
        p_colors=["#D4870A" if k==action else "#D0D7E2" for k,_ in sorted_p]
        fig_pb=go.Figure(go.Bar(x=p_vals,y=p_labels,orientation="h",
            marker_color=p_colors,text=[f"{v:.1f}%" for v in p_vals],
            textposition="inside",textfont=dict(color="#1A1A2E",family="Arial")))
        dark_chart_layout(fig_pb,250,xaxis=dict(range=[0,100],gridcolor="#D0D7E2",title="Confidence %"))
    ...
    if stats:
        ...
        df_h["color"]=df_h["distribution_action"].map(ACTION_COLORS).fillna("#5A6A7E")
        ...
        dark_chart_layout(fig_h,280,yaxis=dict(gridcolor="#D0D7E2",title="Solar Gen (kW)"))
...
def page_billing(user):
    ...
    if payments:
        ...
        fig.add_trace(go.Scatter(x=df_ch["date"],y=df_ch["cumulative"],
            name="Cumulative",line=dict(color="#D4870A",width=2.5),yaxis="y2"))
...
def page_weather(user, api_key, city):
    ...
    fig_f.add_trace(go.Scatter(x=hlabels,y=forecast,name="Predicted kW",
        line=dict(color="#D4870A",width=3),fill="tozeroy",fillcolor="rgba(212,135,10,0.12)"))
    fig_f.add_trace(go.Scatter(x=hlabels,y=[v/100 for v in irr_list],name="Irradiance (scaled)",
        line=dict(color="#CC5500",width=1.5,dash="dot"),yaxis="y2"))
    # Use add_shape + add_annotation instead of add_vline (avoids string-x Plotly bug)
    ...
    fig_f.add_annotation(x=hlabels[cur_h],yref="paper",y=1.02,
        text="▼ NOW",showarrow=False,font=dict(color="#0066CC",size=11,family="Arial"))
    dark_chart_layout(fig_f,340,xaxis=dict(gridcolor="#D0D7E2",title="Hour"),
        yaxis=dict(gridcolor="#D0D7E2",title="kW"),
    ...
    with s1:
        fig_si=go.Figure(go.Scatter(x=cloud_range,y=irr_sens,mode="lines+markers",
            line=dict(color="#D4870A",width=2.5),marker=dict(size=6,color="#CC5500"),
            fill="tozeroy",fillcolor="rgba(212,135,10,0.10)"))
        fig_si.add_shape(type="line",xref="x",yref="paper",
            x0=w["cloud_cover"],x1=w["cloud_cover"],y0=0,y1=1,
            line=dict(color="#0066CC",width=2,dash="dash"))
        fig_si.add_annotation(x=w["cloud_cover"],yref="paper",y=1.04,
            text=f"Now: {w['cloud_cover']:.0f}%",showarrow=False,
            font=dict(color="#0066CC",size=10,family="Arial"))
        dark_chart_layout(fig_si,280,xaxis=dict(gridcolor="#D0D7E2",title="Cloud Cover %"),
            yaxis=dict(gridcolor="#D0D7E2",title="W/m²"))
        fig_si.update_layout(title=dict(text="Cloud Cover → Irradiance",font=dict(color="#5A6A7E",family="Arial",size=11)))
        st.plotly_chart(fig_si,use_container_width=True)
    with s2:
        fig_sk=go.Figure(go.Scatter(x=cloud_range,y=kw_sens,mode="lines+markers",
            line=dict(color="#0066CC",width=2.5),marker=dict(size=6,color="#1A8C4E"),
            fill="tozeroy",fillcolor="rgba(0,102,204,0.08)"))
        fig_sk.add_shape(type="line",xref="x",yref="paper",
            x0=w["cloud_cover"],x1=w["cloud_cover"],y0=0,y1=1,
            line=dict(color="#D4870A",width=2,dash="dash"))
        fig_sk.add_annotation(x=w["cloud_cover"],yref="paper",y=1.04,
            text=f"Now: {sk:.2f} kW",showarrow=False,
            font=dict(color="#D4870A",size=10,family="Arial"))
        dark_chart_layout(fig_sk,280,xaxis=dict(gridcolor="#D0D7E2",title="Cloud Cover %"),
            yaxis=dict(gridcolor="#D0D7E2",title="kW"))
        fig_sk.update_layout(title=dict(text="Cloud Cover → Solar Generation",font=dict(color="#5A6A7E",family="Arial",size=11)))
    ...
    if stats:
        ...
        fig_w.add_trace(go.Scatter(x=df_w["timestamp"],y=df_w["temperature"],
            name="Temperature °C",line=dict(color="#CC5500",width=2)))
        fig_w.add_trace(go.Scatter(x=df_w["timestamp"],y=df_w["humidity"],
            name="Humidity %",line=dict(color="#0066CC",width=2,dash="dot")))
        fig_w.add_trace(go.Scatter(x=df_w["timestamp"],y=df_w["cloud_cover"],
            name="Cloud %",line=dict(color="#5A6A7E",width=2,dash="dashdot")))
