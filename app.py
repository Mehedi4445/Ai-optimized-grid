...
def dark_chart_layout(fig, height=300, **kw):
    ...
    base_xaxis  = dict(gridcolor="#1E3A5F")
    base_yaxis  = dict(gridcolor="#1E3A5F")
    base_legend = dict()
    ...
    fig.update_layout(
        ...
        font=dict(family="Rajdhani"), margin=base_margin,
...
def page_admin(user):
    ...
    if billing_data:
        ...
        kpi_card(bk1,"📋",f"${total_billed:.2f}",    "Total Billed",      "#F5A623")
        kpi_card(bk2,"✅",f"${total_collected:.2f}",  "Total Collected",   "#00FF94")
        kpi_card(bk3,"⚠️",f"${total_outstanding:.2f}","Outstanding",       "#FF3D5A" if total_outstanding>0 else "#00FF94")
        kpi_card(bk4,"🔴",str(unpaid_users),          "Users With Balance","#FF6B00")
        ...
        def highlight_status(val):
            if "Clear"   in str(val): return "color: #00FF94; font-weight:bold"
            if "Partial" in str(val): return "color: #F5A623; font-weight:bold"
            if "Unpaid"  in str(val): return "color: #FF3D5A; font-weight:bold"
            return "color: #7BA3C4"
        ...
        if not df_owed.empty:
            st.markdown("<div style='color:#FF3D5A;font-family:Orbitron;font-size:0.8rem;letter-spacing:2px;margin:1rem 0 0.5rem'>OUTSTANDING BALANCES</div>",
                        unsafe_allow_html=True)
            fig_bill = go.Figure(go.Bar(
                x=df_owed["username"], y=df_owed["outstanding"],
                marker=dict(color=df_owed["outstanding"],
                            colorscale=[[0,"#F5A623"],[1,"#FF3D5A"]],
                            line=dict(color="#050A1A",width=1)),
                text=[f"${v:.2f}" for v in df_owed["outstanding"]],
                textposition="outside",
                textfont=dict(color="#E8F4FD",family="Rajdhani")
            ))
            dark_chart_layout(fig_bill, 280,
                yaxis=dict(gridcolor="#1E3A5F", title="Outstanding ($)"))
            ...
            fig_stack.add_trace(go.Bar(
                ...
                marker_color="#00FF94", opacity=0.85
            ))
            fig_stack.add_trace(go.Bar(
                name="Outstanding", x=df_bill["username"], y=df_bill["outstanding"],
                marker_color="#FF3D5A", opacity=0.85
            ))
...
def page_weather(user, api_key, city):
    ...
    fig_f.add_annotation(x=hlabels[cur_h],yref="paper",y=1.02,
        text="▼ NOW",showarrow=False,font=dict(color="#00D4FF",size=11,family="Orbitron"))
    ...
    with s1:
        ...
        fig_si.add_annotation(x=w["cloud_cover"],yref="paper",y=1.04,
            ...
            font=dict(color="#00D4FF",size=10,family="Rajdhani"))
    ...
    with s2:
        ...
        fig_sk.add_annotation(x=w["cloud_cover"],yref="paper",y=1.04,
            ...
            font=dict(color="#F5A623",size=10,family="Rajdhani"))
